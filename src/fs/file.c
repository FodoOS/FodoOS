...
struct filesystem* filesystems[FODOOS_MAX_FILESYSTEMS];
struct file_descriptor* file_descriptors[FODOOS_MAX_FILE_DESCRIPTORS];

static struct filesystem** fs_get_free_filesystem()
{
  int i = 0;
  for (i = 0; i < FODOOS_MAX_FILESYSTEMS; i++)
  {
    if (filesystems[i] == 0)
    {
...
{
  int res = -ENOMEM;
  for (int i = 0; i < FODOOS_MAX_FILE_DESCRIPTORS; i++)
  {
...
struct filesystem* fs_resolve(struct disk* disk)
{
  ...
  for (int i = 0; i < FODOOS_MAX_FILESYSTEMS; i++)
  {
...
out:
  // fopen shoudln't return negative values
  if (res < 0)
    res = 0;
