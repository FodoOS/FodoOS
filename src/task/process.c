...
static struct process* processes[FODOOS_MAX_PROCESSES] = {};
...
{
  if (process_id < 0 || process_id >= FODOOS_MAX_PROCESSES)
  {
    return NULL;
  }
...
{
  int res = -ENOMEM;
  for (int i = 0; i < FODOOS_MAX_PROGRAM_ALLOCATIONS; i++)
  {
...
{
  int res = 0;
  res = paging_map_to(process->task->page_directory, (void*)FODOOS_PROGRAM_VIRTUAL_ADDRESS, process->ptr, paging_align_address(process->ptr + process->size), PAGING_IS_PRESENT | PAGING_ACCESS_FROM_ALL | PAGING_IS_WRITEABLE);
  return res;
}
...
int process_map_memory(struct process* process)
{
  ...
  paging_map_to(process->task->page_directory, (void*)FODOOS_PROGRAM_VIRTUAL_STACK_ADDRESS_END, process->stack, paging_align_address(process->stack+FODOOS_USER_PROGRAM_STACK_SIZE), PAGING_IS_PRESENT | PAGING_ACCESS_FROM_ALL | PAGING_IS_WRITEABLE);
...
{
  for (int i = 0; i < FODOOS_MAX_PROCESSES; i++)
  {
    if (processes[i] == 0)
      return i;
  }
...
int process_load_for_slot(const char* filename, struct process** process, int process_slot)
{
  ...
  _process->stack = kzalloc(FODOOS_USER_PROGRAM_STACK_SIZE);
  if (!_process->stack)
  {
    res = -ENOMEM;
    goto out;
  ...
  if (ERROR_I(_process->task) == 0)
  {
    ...
    // Task is NULL due to error being returned in task_new
    _process->task = NULL;
    goto out;
  }
...
void process_switch_to_any()
{
  ...
  // Another better choice is to launch shell or another program
